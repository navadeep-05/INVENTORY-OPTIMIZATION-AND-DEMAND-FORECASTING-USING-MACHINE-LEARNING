...
st.set_page_config(page_title="ML Project 1", layout="wide", page_icon = "/workspaces/INVENTORY-OPTIMIZATION-AND-DEMAND-FORECASTING-USING-MACHINE-LEARNING/icon2.png", initial_sidebar_state="expanded")
    ...
    with st.form(key='reset_form'):
        ...
        if reset_button:
            if not is_user_exists(username, email):
                ...
                update_user_password(username if username else is_user_exists[0], new_password)
...
def app():
    ...
    st.image(r"/workspaces/INVENTORY-OPTIMIZATION-AND-DEMAND-FORECASTING-USING-MACHINE-LEARNING/logo2.png", width=100)
    st.header("Welcome to the Web Application")
    st.image(r"/workspaces/INVENTORY-OPTIMIZATION-AND-DEMAND-FORECASTING-USING-MACHINE-LEARNING/banner3.jpeg", use_column_width=True)
    ...
    if uploaded_files:
        if len(uploaded_files) > 5:
            ...
            if evaluate_button or st.session_state.evaluated:
                ...
                selected_model = st.selectbox("Select Model for Inventory Optimization", options=["Random Forest", "ARIMA"], index=0)
                lead_time = 7
...
def show_about():
    ...
    """)
    st.write("")
    st.image(r"/workspaces/INVENTORY-OPTIMIZATION-AND-DEMAND-FORECASTING-USING-MACHINE-LEARNING/future imp2.png", caption='Future Estimates', width=700, clamp=True, channels='RGB')
...
def logout():
    ...
    st.write("")
    st.video(r"/workspaces/INVENTORY-OPTIMIZATION-AND-DEMAND-FORECASTING-USING-MACHINE-LEARNING/an animation of a hand drawn business strategy with chart_preview.mp4")
